...
$koneksi = mysqli_connect("localhost", "root", "password", "db_go_bengkel");

for ($i = 0; $i < count($kabupaten_arr); $i++) {
	
	$nama = $kabupaten_arr[$i]['name'];
	$idprovinsi = ($kabupaten_arr[$i]['province_id']);
	$latitude = $kabupaten_arr[$i]['latitude'];
	$longitude = $kabupaten_arr[$i]['longitude'];

	$cek = mysqli_query($koneksi, "SELECT * FROM kabupaten WHERE nama = '$nama'");
	if(mysqli_num_rows($cek) <= 0) {
		mysqli_query($koneksi, "INSERT INTO kabupaten (nama, idprovinsi, lat, lng) VALUES ('$nama', '$idprovinsi', '$latitude', '$longitude')");
	}
}
