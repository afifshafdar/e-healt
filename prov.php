...
for ($i = 0; $i < count($provinsi_arr); $i++) {
	$idprovinsi = $provinsi_arr[$i]['id'];
	$nama = $provinsi_arr[$i]['name'];
	$latitude = $provinsi_arr[$i]['latitude'];
	$longitude = $provinsi_arr[$i]['longitude'];

	$cek = mysqli_query($koneksi, "SELECT * FROM provinsi WHERE nama = '$nama'");
	if(mysqli_num_rows($cek) <= 0) {
		mysqli_query($koneksi, "INSERT INTO provinsi (idprovinsi, nama, lat, lng) VALUES ('$idprovinsi', '$nama', '$latitude', '$longitude')");
	}
}
